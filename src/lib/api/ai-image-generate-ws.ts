...
/** 与后端约定的生图 WebSocket 消息 */
export type AiGenerateImageWsMessage = {
  type: 'ai_generate';
  model: string;
  function: 'image';
  params: {
...
/**
 * 生图 WebSocket 地址：`{upstream}/websocket`，登录时附带 `satoken` 查询参数。
 */
export function getAiImageGenerateWebSocketUrl(): string {
...
export function buildAiImageGenerateMessage(args: {
  model: string;
  prompt: string;
  aspectRatio?: string;
  resolution?: string;
  imageUrls: string[];
}): AiGenerateImageWsMessage {
  const params: AiGenerateImageWsMessage['params'] = {
    prompt: args.prompt,
...
export type AiGenerateWsHandleResult =
  ...
  | { kind: 'success'; sessionId?: string; imageUrls: string[]; costUsd?: string }
...
function readImageUrlsFromPayload(payload: AiGenerateWsPayload): string[] {
  const urls: string[] = [];
...
/**
 * 解析生图 WebSocket 下行消息。
 * 兼容旧版 `dataList[].fileUrl` 与新版 `task_status` + `output[].image`。
 */
export function handleAiGenerateWebSocketMessage(
  ...
  const imageUrls = readImageUrlsFromPayload(payload);
  ...
  if (isSuccessStatus || imageUrls.length > 0) {
    if (imageUrls.length === 0) {
      return { kind: 'error', message: messages.noResultUrl };
    }
    return { kind: 'success', sessionId, imageUrls, costUsd };
  }
