...
| 字段 | 类型 | 必填 | 本模型取值 | 说明 |
|------|------|------|------------|------|
| `model` | string | 是 | `music-lyrics` | 模型 ID。 |
| `function` | string | 是 | `audio` | 须与本模型能力一致。 |
| `input` | object | 是 | 见下文 | 参数结构见下一节。 |
| `webhook` | string | 否 | （可选） | 回调地址；可不传，详见参考文档。 |

---

## 本模型 **input** 字段（音频相关：`function` 为 `audio`）

下列字段位于请求体中的 **`input`** 对象内（写法与 **create-a-prediction.md** 各能力章节一致，以下为 **本模型** 实际支持的子集与约束）。

_（本模型未配置 `schema.input`，请参考 [create-a-prediction.md](/model/create-a-prediction.md) 中与当前 `function` 对应的通用说明，并结合接口返回调整参数。）_
