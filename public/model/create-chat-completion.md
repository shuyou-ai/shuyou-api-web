...
#### reasoning `string` （ShuYou 拓展字段）

推理过程的文本内容，用于展示模型对答案的思考过程或中间分析。实际是否返回取决于模型和请求中的推理配置。

#### reasoning_details `string`（ShuYou 拓展字段）
...
- type `string`  
  工具类型。目前标准为 `function`，ShuYou 可能在扩展中支持 `custom` 等其他类型。
- function `object`  
...
指定用于处理请求的服务类型或服务等级。ShuYou 自身不限定取值，如果上游模型返回该字段则会透传。
...
- reasoning_content `string`（ShuYou 拓展字段）  
...
此指纹表示本次请求使用的后端配置。虽然在部分上游已标记为 Deprecated，但 ShuYou 仍会保留并透传该字段。
...
- **背景**：Claude Opus 4.5 原生使用 Anthropic Messages 协议，ShuYou 将 Chat Completion 协议转换为 Messages 协议。
